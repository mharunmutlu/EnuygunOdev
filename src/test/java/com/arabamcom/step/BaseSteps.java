...
public class BaseSteps extends BaseTest {
    ...
    public void checkelementtext(String key){
        compareText = "10:00 ile 17:00 arası";
    ...
    @Step("<key> elementi ucuzdan pahalıya mı sıralı kontrol et" )
    public void checkPrice(String key){
        List<WebElement> elements = findElements(key);
        for (int i = 0 ; i < elements.size() ; ) { //eğer güncel gün seçilirse i++ olduğu durumda fail oluyor.
            for (int x = 1; x < elements.size();)
            {
                if (i >= elements.size())
                    break;
                double price = Double.parseDouble(elements.get(i).getAttribute("data-price"));
                double price2 = Double.parseDouble(elements.get(x).getAttribute("data-price"));
                if (price < price2){
    ...
    @Step("Yetişkin sayısını <key> seç")
    public void increaseAdult(String key){
        WebElement element = findElement("Yetiskin_Sayisi");
    ...
    public void increaseKid(String key){
        WebElement element = findElement("Cocuk_Sayisi");
        int adet = Integer.parseInt(key);
        for (int i = 0; i < adet; i++)
            element.click();
    }

    @Step("Bebek sayısını <key> seç")
    public void increasebabe(String key){
        WebElement element = findElement("Bebek_Sayisi");
        int adet = Integer.parseInt(key);
        for (int i = 0; i < adet; i++)
            element.click();
    }
